#!/usr/bin/env python3
"""Runner for pipeline 1."""

import json
import time
import sys
import archive.cataloger.pipeline_runs.pipeline_1 as pipeline_1
...
def main():
    ...
    print(f"Starting pipeline 1...")

    start_time = time.time()

    try:
        result = pipeline_1.classify_catalog(catalog)
        ...
        runtime = time.time() - start_time
        print(f"\nPipeline 1 FAILED after {runtime:.2f}s")
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
