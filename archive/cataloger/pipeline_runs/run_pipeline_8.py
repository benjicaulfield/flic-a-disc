#!/usr/bin/env python3
"""Runner for pipeline 8."""

import json
import time
import sys
import archive.cataloger.pipeline_runs.pipeline_8 as pipeline_8
...
def main():
    ...
    print(f"Starting pipeline 8...")

    start_time = time.time()

    try:
        result = pipeline_8.classify_catalog(catalog)
        runtime = time.time() - start_time

        print(f"\nPipeline 8 COMPLETED!")
        ...
        with open('results_8.json', 'w') as f:
        ...
        runtime = time.time() - start_time
        print(f"\nPipeline 8 FAILED after {runtime:.2f}s")
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
