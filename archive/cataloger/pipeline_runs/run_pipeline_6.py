#!/usr/bin/env python3
"""Runner for pipeline 6."""

import json
import time
import sys
import archive.cataloger.pipeline_runs.pipeline_6 as pipeline_6
...
def main():
    ...
    print(f"Starting pipeline 6...")

    start_time = time.time()

    try:
        result = pipeline_6.classify_catalog(catalog)
        ...
        runtime = time.time() - start_time
        print(f"\nPipeline 6 FAILED after {runtime:.2f}s")
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
