...
import type { DiscogsRecord, MLData, PerformanceStats } from "../../types";
import { Paginate } from '../../hooks/Paginate';
import { apiFetch, mlFetch } from "../../api/client";

const DiscogsCatalogTraining = () => {
  ...
  const loadPage = async () => {
    try {
      setLoading(true);
      const response = await mlFetch("/discogs/select-batch/", {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const sortListingsByAgreement = () => {
    ...
    const categorized = currentPageRecords.map((listing, index) => {
      ...
      const modelSelected = mlData.mean_predictions[index] > (mlData.threshold ?? 0.5);
...
export default DiscogsCatalogTraining;
